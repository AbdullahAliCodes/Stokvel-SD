...
export default function QuickPayModal({
  ...
  const [paystackReady, setPaystackReady] = useState(
    Boolean(window.PaystackPop?.setup),
  );
  ...
  const paystackAmount =
    Number.isFinite(parsedAmount) && parsedAmount > 0 ? parsedAmount * 100 : 0;
  ...
  async function verifyAndRecord(transaction, parsedAmountLocal) {
    ...
    try {
      ...
      if (!json?.success)
        throw new Error("Payment was verified but could not be recorded.");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 p-4">
      <div className={`${cardLight} w-full max-w-sm p-6`}>
        ...
        <p className="mb-4 text-sm text-stone-600">
          Contributing to{" "}
          <strong className="text-stone-800">{groupName}</strong>
        </p>
        <input
          type="number"
          min="1"
        ...
        />
        {error && (
          <p className="mb-3 text-xs text-red-700 dark:text-red-300">{error}</p>
        )}
        <div className="flex gap-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className={`${btnSecondary} flex-1 py-2 text-sm`}
          >
            Cancel
          </button>
        </div>
