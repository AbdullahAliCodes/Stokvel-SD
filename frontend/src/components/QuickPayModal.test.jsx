...
describe("QuickPayModal", () => {
  ...
  it("handles not-ready paystack state", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Loading payment…" }),
    ).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Loading payment…" }));
    expect(
      screen.getByText("Paystack is still loading. Please try again."),
    ).toBeInTheDocument();
  });
  ...
  it("submits successful payment and calls onSuccess + onClose", async () => {
    ...
    fetch.mockResolvedValue({
      ...
      status: 200,
      text: async () =>
        JSON.stringify({ success: true, contribution: { id: "c1" } }),
    });
  ...
  it("shows API error message from JSON payload and calls onRecordError", async () => {
    ...
    fetch.mockResolvedValue({
      ...
      status: 400,
      text: async () =>
        JSON.stringify({ error: "Payment outside valid window" }),
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Payment outside valid window"),
      ).toBeInTheDocument();
      expect(props.onRecordError).toHaveBeenCalledWith(
        "Payment outside valid window",
      );
    });
  });
