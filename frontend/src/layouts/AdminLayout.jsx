...
export default function AdminLayout() {
  ...
  return (
    <div className="box-border flex h-dvh min-h-0 w-full flex-col gap-3 overflow-hidden bg-[#F4F5F0] p-3 text-stone-800 dark:bg-slate-950 dark:text-stone-100 md:flex-row md:gap-4 md:p-4">
      <aside
        className={`${cardLight} stkg-no-lift flex w-full shrink-0 flex-col overflow-hidden border-t-4 border-emerald-700 md:h-full md:w-[240px] md:min-w-[240px] md:max-w-[240px]`}
      >
        ...
        <div className="shrink-0 space-y-2 border-t border-stone-200 p-3 dark:border-slate-700">
          ...
          <button
            type="button"
            className="stkg-btn flex w-full items-center justify-center gap-2 rounded-lg border border-stone-300 bg-white py-2 text-sm font-medium text-stone-700 shadow-sm transition hover:bg-stone-100 dark:border-slate-600 dark:bg-slate-800 dark:text-stone-200 dark:hover:bg-slate-700"
            onClick={async () => {
