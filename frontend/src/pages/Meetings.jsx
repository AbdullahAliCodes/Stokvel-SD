...
export default function Meetings() {
    ...
    async function load() {
      ...
      try {
        ...
        writeViewCache(cacheKey, {
          ...
          members: nextMembers,
          totalContribution: Number(json.totalContribution ?? prev?.totalContribution ?? 0),
  ...
  const canManageMeetings = ['treasurer', 'admin'].includes(String(myGroupRole || '').toLowerCase())

  const { upcomingMeetings, pastMeetings } = useMemo(() => {
    const nowTs = Date.now()
    const upcoming = meetings.filter((m) => new Date(m.meeting_date).getTime() >= nowTs)
    const past = meetings.filter((m) => new Date(m.meeting_date).getTime() < nowTs)
    upcoming.sort(
  ...
  async function handleSaveMeeting(meetingId) {
    ...
    setMeetingActionOk('')
    try {
      const res = await fetch(apiUrl(`/api/stokvels/${stokvel_id}/meetings/${meetingId}`), {
        method: 'PATCH',
        headers: {
  ...
  async function handleSaveMinutes(meetingId) {
    ...
    setMeetingActionOk('')
    try {
      const res = await fetch(apiUrl(`/api/stokvels/${stokvel_id}/meetings/${meetingId}/minutes`), {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${session.access_token}`,
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-2 border-b border-stone-100 pb-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-stone-500">{toDisplayDate(meeting.meeting_date)}</p>
          </div>
          <Link
        ...
        {editingMeetingId === meeting.id ? (
          <div className="space-y-2 rounded-lg bg-stone-50/80 p-3">
            ...
            <input
              ...
              onChange={(e) => setEditDraft((prev) => ({ ...prev, meetingDate: e.target.value }))}
              className={inputLight}
            />
            <input
              type="url"
              value={editDraft.meetingLink ?? ''}
              onChange={(e) => setEditDraft((prev) => ({ ...prev, meetingLink: e.target.value }))}
              className={inputLight}
          ...
          <>
            ...
            {canManageMeetings ? (
              <div className="space-y-2 border-t border-stone-200 pt-3">
                <button type="button" onClick={() => openEdit(meeting)} className={btnSecondary}>
                  Edit meeting
                </button>
                <button
                  type="button"
    ...
    <div className="space-y-8">
      <header className="border-b border-stone-200 pb-4">
        ...
        <p className={`${pageSubtitle} text-stone-600`}>
          <span className="font-medium text-stone-800">{groupName}</span> — upcoming and past
          sessions. Agenda and minutes are shown on each card.
        </p>
      </header>
      ...
      {meetingActionError ? <p className={`text-sm ${errorBox}`}>{meetingActionError}</p> : null}
      {meetingActionOk ? (
      ...
      <section>
        <h2 className="mb-4 text-lg font-bold text-emerald-800">Upcoming meetings</h2>
        {loading && meetings.length === 0 ? (
          <p className="text-sm text-stone-500">Loading…</p>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {upcomingMeetings.map((m) => renderMeetingCard(m, { isPast: false }))}
      ...
      <section>
        <h2 className="mb-4 text-lg font-bold text-emerald-800">Past meetings</h2>
        <div className="grid gap-4 md:grid-cols-2">
