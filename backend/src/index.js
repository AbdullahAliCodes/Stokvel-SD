...
const DOCUMENTS_BUCKET =
  process.env.SUPABASE_DOCUMENTS_BUCKET || "stokvel-documents";
const documentsUpload = multer({
  storage: multer.memoryStorage(),
  limits: { fileSize: 10 * 1024 * 1024, files: 10 },
});

console.log("Testing Codecov");
...
function normalizeMemberDetails(raw, limit = 500) {
  ...
  return raw
    .map((m) => {
      ...
      const userId = UUID_RE_MEMBER_DETAILS.test(maybeUid)
        ? maybeUid.trim().toLowerCase()
        : "";
      return {
        userId,
...
function collectUuidsFromMemberDetails(details) {
  ...
  for (const row of details ?? []) {
    ...
    if (typeof uid !== "string" || !UUID_RE_MEMBER_DETAILS.test(uid.trim()))
      continue;
...
app.get("/api/public/stokvels", async (_req, res) => {
  try {
    ...
    const { data, error } = await svc
      .from("stokvels")
      .select(
        "id, name, type, contribution_amount, members_count, cycle_length, created_at",
      )
      .eq("is_public", true)
      .eq("status", "active")
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: error.message || "Failed to load public stokvels." });
    }
  ...
  documentsUpload.array("documents", 10)(req, res, async (err) => {
    ...
    try {
      ...
      if (!svc) {
        return res.status(500).json({
          error:
            "Document upload requires SUPABASE_SERVICE_ROLE_KEY on the API server.",
        });
      }
      ...
      const { error: bucketError } = await svc.storage.createBucket(
        DOCUMENTS_BUCKET,
        {
          public: true,
...
app.get("/api/my-stokvels", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (cached) {
      console.log(
        `[perf] GET /api/my-stokvels ${elapsedMs(started).toFixed(1)}ms (cache) user=${req.user.id}`,
      );
    ...
    res.json(payload);
    console.log(
      `[perf] GET /api/my-stokvels ${elapsedMs(started).toFixed(1)}ms user=${req.user.id}`,
    );
  } catch (err) {
...
app.get("/api/my-meetings", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (cached) {
      console.log(
        `[perf] GET /api/my-meetings ${elapsedMs(started).toFixed(1)}ms (cache) user=${req.user.id}`,
      );
      return res.json(cached);
    }
    ...
    const stokvelIds = [
      ...new Set((memberships ?? []).map((m) => m.stokvel_id).filter(Boolean)),
    ];
    if (stokvelIds.length === 0) {
      console.log(
        `[perf] GET /api/my-meetings ${elapsedMs(started).toFixed(1)}ms user=${req.user.id}`,
      );
    ...
    const [meetingsRes, groupsRes] = await Promise.all([
      userSupabase
        .from("meetings")
        .select(
          "id, stokvel_id, title, meeting_date, meeting_link, agenda, minutes, notes, created_at",
        )
    ...
    const groupNameById = new Map(
      (groupsRes.data ?? []).map((g) => [g.id, g.name || "Unnamed group"]),
    );
...
app.post("/api/stokvels", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (treasurerUuid === creatorId) {
      return res
        .status(400)
        .json({ error: "You cannot designate yourself as treasurer." });
    }

    const parsedDetails = normalizeMemberDetails(
      memberDetails,
      parsedMembersCount ?? 500,
    );
    ...
    const { error: creatorMemberErr } = await userSupabase
      .from("stokvel_members")
      .insert([
        {
    ...
    if (creatorMemberErr) {
      ...
      return res.status(500).json({
        error:
          creatorMemberErr.message ||
          "Failed to assign creator as group admin.",
      });
    }
    ...
    if (!svc) {
      console.error(
        "POST /api/stokvels: SUPABASE_SERVICE_ROLE_KEY required for SoD member setup.",
      );
      await userSupabase
        .from("stokvel_members")
        .delete()
    ...
    try {
      ...
      for (const row of parsedDetails) {
        ...
        if (
          row.userId &&
          UUID_RE_MEMBER_DETAILS.test(String(row.userId).trim())
        )
          continue;
      ...
      const { error: syncErr } = await ensurePlatformAdminsInStokvel(
        svc,
        stokvelId,
      );
    ...
    console.log(
      `[perf] POST /api/stokvels ${elapsedMs(started).toFixed(1)}ms user=${req.user.id}`,
    );
  } catch (err) {
...
app.post("/api/stokvels/:id/join", requireAuth, async (req, res) => {
  try {
    ...
    if (stokvelErr) {
      ...
      return res
        .status(500)
        .json({ error: stokvelErr.message || "Failed to verify stokvel." });
    }
    if (!stokvelRow?.id) {
      return res
        .status(400)
    ...
    if (memberLookupErr) {
      console.error(
        "POST /api/stokvels/:id/join membership lookup:",
        memberLookupErr,
      );
      return res.status(500).json({
        error: memberLookupErr.message || "Failed to verify membership.",
      });
    }
