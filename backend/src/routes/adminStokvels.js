...
function createUserScopedClient(req) {
  return createUserJwtSupabase(req, 'adminStokvels')
}
...
function rowMatchesPatch(row, patch) {
  ...
  for (const [key, val] of Object.entries(patch)) {
    ...
    if (
      val != null &&
      got != null &&
      String(got).toLowerCase() === String(val).toLowerCase()
    )
      continue;
 ...
 */
async function applyMemberDetailEmailsToProfiles(
  client,
  memberDetails,
  allowedUserIds,
) {
  ...
  for (const row of memberDetails) {
    ...
    if (upErr) {
      console.error(
        "applyMemberDetailEmailsToProfiles update:",
        uid,
        upErr.message,
      );
    }
...
async function getGroupAndCreatorContact(client, stokvelId) {
  const { data: stokvel, error: stokvelError } = await client
    ...
    .eq("id", stokvelId)
    .maybeSingle();
  if (stokvelError || !stokvel)
    return { stokvel: null, creatorEmail: "", creatorId: null };

  const createdBy =
    typeof stokvel.created_by === "string" ? stokvel.created_by : "";
...
router.get("/users", requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    const firstErr =
      byFirst.error || byLast.error || byUsername.error || byEmail.error;
    if (firstErr) {
  ...
  try {
    ...
    if (existing) {
      return res
        ...
        .json({
          error:
            "A stokvel with this name already exists. Choose a different name.",
        });
    }

    const membersCountNorm = normalizeMembersCount(membersCount);
    const normalizedMemberDetails = normalizeMemberDetails(
      memberDetails,
      membersCountNorm ?? 500,
    );

    const potIn = body.payout_order_type ?? body.payoutOrderType ?? payoutOrder;
    const payoutOrderType =
      typeof potIn === "string" && potIn.toLowerCase() === "manual"
        ? "manual"
        : "randomize";
    const proposedRaw =
      body.proposed_payout_sequence ?? body.proposedPayoutSequence;
    ...
    const insertRow = {
      ...
      meeting_frequency:
        typeof meetingFrequency === "string" ? meetingFrequency : "monthly",
      cycle_length: cycle,
    ...
    const requestedTreasurerId =
      normalizeTreasurerUserId(treasurerUserId) || req.user.id;
    ...
    const creatorRole =
      requestedTreasurerId === req.user.id ? "treasurer" : "admin";
    ...
    if (memberError) {
      ...
      const constraintHint = String(memberError.message || "").includes(
        "group_role_check",
      )
      ...
      return res.status(500).json({
        error:
          (memberError.message ||
            "Failed to add you as the first member; the group was not created.") +
          constraintHint,
      });
    }
    ...
    await applyMemberDetailEmailsToProfiles(
      client,
      normalizedMemberDetails,
      allowedProfileEmailIds,
    );

    // Fire-and-forget style notifications for members added during create.
    const allAddedIds = normalizeInitialMemberIds(
      initialMemberIds,
      req.user.id,
    );
    ...
    if (!act.ok) {
      ...
      await client
        ...
        .delete()
        .eq("stokvel_id", stokvel.id);
      const { error: rbDel } = await client
        .from("stokvels")
        .delete()
        .eq("id", stokvel.id);
    ...
    return res
      .status(201)
      .json({ success: true, stokvel: stokvelOut ?? stokvel });
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const raw = rawUsername || identifier || rawLegacy || "";
      ...
      if (already) {
        return res
          .status(409)
          .json({ error: "That user is already a member of this stokvel." });
      }

      const invitedGroupRole = await groupRoleForUserProfile(
        client,
        targetUserId,
      );
...
router.post(
  "/stokvels/:stokvelId/invitations",
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
      ...
      const { data: invite, error: inviteError } = await createInvitation(
        client,
        {
          stokvelId,
          email,
    ...
    try {
      ...
      if (typeof body.name === "string") {
        ...
        if (dup) {
          return res
            .status(409)
            .json({ error: "Another stokvel already uses this name." });
        }
        patch.name = trimmed;
      }
      ...
      if (body.proposedPayoutSequence !== undefined) {
        if (!Array.isArray(body.proposedPayoutSequence)) {
          return res
            .status(400)
            .json({
              error: "proposedPayoutSequence must be an array of UUID strings",
            });
        }
        patch.proposed_payout_sequence = body.proposedPayoutSequence.filter(
          (id) => typeof id === "string",
        );
      }
      ...
      if (body.cycleLength !== undefined) {
        ...
        if (!Number.isInteger(cycle) || cycle < 1 || cycle > 240) {
          return res
            .status(400)
            .json({
              error: "Cycle length must be an integer between 1 and 240",
            });
        }
      ...
      if (activating) {
        const { data: preRow } = await client
          ...
          .eq("id", stokvelId)
          .maybeSingle();
        const groupNameForInvites =
          typeof preRow?.name === "string" ? preRow.name : "";
        ...
        if (!invitesError && Array.isArray(pendingInvites)) {
          for (const invite of pendingInvites) {
            ...
            if (profile?.id) {
              const role =
                inviteRole ||
                (await groupRoleForUserProfile(client, profile.id));
      ...
      if (Object.keys(patch).length > 0) {
        const { stokvel: upRow, error: upErr } =
          await updateStokvelReturningRow(client, stokvelId, patch);

        if (upErr) {
          console.error("PATCH /api/admin/stokvels:", upErr);
          return res
            .status(500)
            .json({ error: upErr.message || String(upErr) || "Update failed" });
        }

        if (!upRow) {
          return res
            .status(404)
            .json({ error: "Stokvel not found after update." });
        }
      }
      ...
      if (activating) {
        ...
        if (!act.ok) {
          return res.status(400).json({
            error:
              act.error ||
              "Activation failed (payout schedule or member roster).",
          });
        }
      }
      ...
      if (body.status === "active" || body.status === "rejected") {
        const { creatorEmail } = await getGroupAndCreatorContact(
          client,
          stokvelId,
        );
  ...
  async (req, res) => {
    try {
      ...
      const { error: memberDeleteErr } = await client
        ...
        .eq("stokvel_id", stokvelId);
      if (memberDeleteErr)
        return res.status(500).json({ error: memberDeleteErr.message });
      ...
      if (invitationDeleteErr)
        return res.status(500).json({ error: invitationDeleteErr.message });
      ...
      const { error: meetingDeleteErr } = await client
        ...
        .eq("stokvel_id", stokvelId);
      if (meetingDeleteErr)
        return res.status(500).json({ error: meetingDeleteErr.message });

      const { error: contributionDeleteErr } = await client
        .from("contributions")
        .delete()
        .eq("stokvel_id", stokvelId);
      if (contributionDeleteErr)
        return res.status(500).json({ error: contributionDeleteErr.message });
      ...
      const { error: payoutDeleteErr } = await client
        ...
        .eq("stokvel_id", stokvelId);
      if (payoutDeleteErr)
        return res.status(500).json({ error: payoutDeleteErr.message });
      ...
      const { error: issueDeleteErr } = await client
        ...
        .eq("stokvel_id", stokvelId);
      if (issueDeleteErr)
        return res.status(500).json({ error: issueDeleteErr.message });

      const { error: stokvelDeleteErr } = await client
        .from("stokvels")
        .delete()
        .eq("id", stokvelId);
      if (stokvelDeleteErr)
        return res.status(500).json({ error: stokvelDeleteErr.message });
